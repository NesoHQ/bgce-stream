        ...
        <div className="flex flex-col h-full">
            ...
            <ScrollArea className="flex-1 px-4">
                <div className="space-y-3">
                    {users.map((user) => (
                        <div key={user.id} className="flex items-center justify-between p-2 rounded-md hover:bg-muted/50 transition">
                            ...
                            <div className="flex items-center gap-2">
                                {user.role === 'host' && <Badge variant="secondary" className="text-[10px]">HOST</Badge>}
                                {user.isMuted ? <MicOff className="h-4 w-4 text-muted-foreground" /> : <Mic className="h-4 w-4 text-green-500" />}
                            </div>
                        </div>
                    ))}
                </div>
