...
export default function VoiceRoom() {
  ...
  const { me, users, messages, toggleMic, sendMessage } = useMockRoom(
    id,
    "voice",
    "participant"
  );
        ...
        <div className="flex-1 p-6 overflow-y-auto">
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 max-w-6xl mx-auto">
            {users.map((user) => (
              <div
                key={user.id}
                className="aspect-square bg-muted/30 rounded-2xl flex flex-col items-center justify-center relative border-2 border-transparent transition-all hover:bg-muted/50"
              >
